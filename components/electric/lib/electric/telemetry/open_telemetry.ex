defmodule Electric.Telemetry.OpenTelemetry do
  ...
  def setup do
    ...
    # setup_telemetry_poller_handler()
  ...
  # This assumes that a :telemetry_poller process is started as part of the OTP application
  # supervision tree. Currently, it is started as a child of `Electric.Telemetry`.
  defp setup_telemetry_poller_handler do
    :telemetry.attach_many(
      ...
      &__MODULE__.handle_poller_event/4,
      nil
    )
  end
  ...
  def async_with_span(span_ctx \\ nil, name, attributes, fun) do
